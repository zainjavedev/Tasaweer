...
const AUTH_TOKEN_KEY = 'tasaweers_auth_token';
...
const ChatGenerator: React.FC<ChatGeneratorProps> = ({ onUsePrompt, onApiKeyError }) => {
    ...
    useEffect(() => {
        if (!GEMINI_API_KEY) {
            setError("Missing GEMINI_API_KEY in environment.");
            onApiKeyError();
            return;
        }
        try {
...
const App: React.FC = () => {
    const [error, setError] = useState<string | null>(GEMINI_API_KEY ? null : "GEMINI_API_KEY is missing. Set it in your .env.local or .env file.");
    ...
    const handleLogin = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (username === 'admin' && password === 'Pakistan@23') {
    ...
    const handleApiKeyError = useCallback(() => {
        setError("Your GEMINI_API_KEY may be invalid or missing. Update it in your environment and reload the app.");
    }, []);
    ...
    const renderMissingKeyPrompt = () => (
        <div className="flex flex-col items-center justify-center h-screen p-4 text-center">
            <div className="max-w-md bg-white border border-gray-300 p-8 rounded-lg shadow-xl">
                <h2 className="text-2xl font-bold mb-4 text-black">Set GEMINI_API_KEY to Begin</h2>
                <p className="mb-6 text-gray-600">Add GEMINI_API_KEY to your .env.local (or .env) file and restart the dev server.</p>
                <p className="text-xs text-gray-500 mt-4">You can create a key at <a href="https://ai.google.dev/gemini-api/docs/get-started" target="_blank" rel="noopener noreferrer" className="underline hover:text-black">ai.google.dev/gemini-api/docs/get-started</a>.</p>
            </div>
        </div>
    );

    if (!GEMINI_API_KEY) {
        return renderMissingKeyPrompt();
    }

    if (!authToken) {
        return (
            <div className="min-h-screen bg-white text-black flex items-center justify-center p-4">
                <div className="w-full max-w-md bg-white border border-gray-300 p-8 rounded-xl shadow-xl space-y-6">
                    <div className="text-center">
                        <h2 className="text-2xl font-bold text-black">Sign In</h2>
                        <p className="text-gray-600 mt-2 text-sm">Enter the admin credentials to continue.</p>
                    </div>
                    {loginError && <div className="text-red-700 bg-red-100 p-3 rounded-lg text-sm">{loginError}</div>}
                    <form onSubmit={handleLogin} className="space-y-4">
                        <div className="space-y-1">
                            <label className="block text-sm font-semibold text-gray-700" htmlFor="username">Username</label>
                            <input
                                id="username"
                                type="text"
                                value={loginForm.username}
                                onChange={(e) => setLoginForm(prev => ({ ...prev, username: e.target.value }))}
                                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-black focus:border-black bg-white text-black"
                                placeholder="admin"
        ...
        <div className="min-h-screen bg-white text-black flex flex-col items-center p-2 sm:p-4 lg:p-8">
            <div className="w-full max-w-7xl">
                ...
                <div className="flex justify-end mb-4">
                    <button onClick={handleLogout} className="text-sm text-gray-600 underline hover:text-black">Logout</button>
                </div>

                {error && <div className="mb-6 text-red-700 bg-red-100 p-3 rounded-lg text-sm"><p className="font-bold">API Key Error</p><p>{error}</p></div>}
